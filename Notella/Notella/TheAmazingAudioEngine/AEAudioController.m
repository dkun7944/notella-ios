...
- (void)configureChannelsInRange:(NSRange)range forGroup:(AEChannelGroupRef)group {
    ...
    for ( int i = (int)range.location; i < range.location+range.length; i++ ) {
        ...
        if ( channel->type == kChannelTypeChannel ) {
            ...
            if ( /* DISABLES CODE */ (1) /* workaround for graph bug: http://wiki.theamazingaudioengine.com/graph-node-input-callback-bug */
                    || /* DISABLES CODE */ (!hasUpstreamInteraction) || upstreamInteraction.nodeInteractionType != kAUNodeInteraction_InputCallback || memcmp(&upstreamInteraction.nodeInteraction.inputCallback.cback, &rcbs, sizeof(rcbs)) != 0 ) {
                if ( hasUpstreamInteraction ) {
                    checkResult(AUGraphDisconnectNodeInput(_audioGraph, targetNode, targetBus), "AUGraphDisconnectNodeInput");
                }
